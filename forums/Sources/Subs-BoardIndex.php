		...
		if ($boardIndexOptions['include_categories'])
		{
			...
			if (empty($categories[$row_board['id_cat']]))
			{
				$categories[$row_board['id_cat']] = array(
					...
					'collapse_image' => isset($row_board['can_collapse']) ? '<img src="' . $settings['images_url'] . '/' . $context['theme_variant_url'] . ($row_board['is_collapsed'] > 0 ? 'expand_icon.png" alt="+"' : 'collapse_icon.png" alt="-"') . ' />' : '',
