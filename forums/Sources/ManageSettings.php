...
function ModifyModSettings()
{
	...
	$subActions = array(
		...
		'content' => 'ModifyContentModSettings',
...
function ModifyContentModSettings($return_config = false)
{
	global $txt, $scripturl, $context, $settings, $sc, $modSettings;

	$config_vars = array(
		array(
			'large_text', // the data type of the settings, may be one of (int, float, text, large_text, check, select)
			'heaven_box_text', // the name of the setting for the code I posted above the name would be for example, 'my_mod_setting'
			0, // size of the text field, by default for floats and integers the size is 6 and 0 for anything else
			'heaven_box_text' // the label to use for the field, to determine the label, by default it will first check for the key 'text_label', if that hasn't been set then it will check for the variable $txt['name_of_setting'] and then if that isn't set then it will check for the 4th key in the array as seen here
		),
		array(
			'check', // the data type of the settings, may be one of (int, float, text, large_text, check, select)
			'heaven_box_use_twitter', // the name of the setting for the code I posted above the name would be for example, 'my_mod_setting'
			0, // size of the text field, by default for floats and integers the size is 6 and 0 for anything else
			'heaven_box_use_twitter' // the label to use for the field, to determine the label, by default it will first check for the key 'text_label', if that hasn't been set then it will check for the variable $txt['name_of_setting'] and then if that isn't set then it will check for the 4th key in the array as seen here
		),
		array(
			'text', // the data type of the settings, may be one of (int, float, text, large_text, check, select)
			'heaven_box_twitter_username', // the name of the setting for the code I posted above the name would be for example, 'my_mod_setting'
			0, // size of the text field, by default for floats and integers the size is 6 and 0 for anything else
			'heaven_box_twitter_username' // the label to use for the field, to determine the label, by default it will first check for the key 'text_label', if that hasn't been set then it will check for the variable $txt['name_of_setting'] and then if that isn't set then it will check for the 4th key in the array as seen here
		),
		array(
			'int', // the data type of the settings, may be one of (int, float, text, large_text, check, select)
			'heaven_box_twitter_number', // the name of the setting for the code I posted above the name would be for example, 'my_mod_setting'
			6, // size of the text field, by default for floats and integers the size is 6 and 0 for anything else
			'heaven_box_twitter_number' // the label to use for the field, to determine the label, by default it will first check for the key 'text_label', if that hasn't been set then it will check for the variable $txt['name_of_setting'] and then if that isn't set then it will check for the 4th key in the array as seen here
		),
		'',
		array(
			'large_text', // the data type of the settings, may be one of (int, float, text, large_text, check, select)
			'about_us_footer_text', // the name of the setting for the code I posted above the name would be for example, 'my_mod_setting'
			0, // size of the text field, by default for floats and integers the size is 6 and 0 for anything else
			'about_us_footer_text' // the label to use for the field, to determine the label, by default it will first check for the key 'text_label', if that hasn't been set then it will check for the variable $txt['name_of_setting'] and then if that isn't set then it will check for the 4th key in the array as seen here
		),
	);
	...
	$context['post_url'] = $scripturl . '?action=admin;area=modsettings;save;sa=content';
	$context['settings_title'] = $txt['mods_cat_modifications_content'];

	if (isset($_GET['save']))
	{
		checkSession();
		saveDBSettings($config_vars);
		redirectexit('action=admin;area=modsettings;sa=content');
	}
