...
function template_main()
{
	...
	if (!empty($context['boards']) && (!empty($options['show_children']) || $context['start'] == 0))
	{
		...
		<div class="cat_bar">
			<div class="catbg">
				<div class="cat_bar_title">', $txt['parent_boards'] ,'</div>
			</div>
		</div>
		';
	...
	<div class="tborder topic_table" id="messageindex">
		<table class="MessageIndex_table_grid" cellspacing="0">
			<tbody>';
		...
		{
			$alternate = (isset($alternate)) ? !$alternate : false;
			...
			{
				$default_color_class = 'stickybg';
			
				if(!$alternate)
				{
					$alt_thingy = '2';
				
					$default_color_class = $default_color_class.$alt_thingy;
				}
			...
			{
				$default_color_class = 'MessageIndex_topic_row';
			...
			// figure out the stupid topic icon since the system smf has is DUMB and confusing: MLM -VisualPulse.net
			$topic_icon = '';
			...
			if($topic['is_sticky'])
			{
				if($topic_icon == 'new' || $topic_icon == 'new_locked' || $topic_icon == 'locked')
				{
					$topic_icon = $topic_icon.'_sticky';
				}
				else
				{
					$topic_icon = 'sticky';
			...
			echo '
				<tr class="', $color_class ,'">
					<td class="icon1">
						<img src="', $settings['images_url'], '/thread_icons/', $topic_icon , '.png" alt="', $topic_icon , '" />
					</td>
			...
			echo '
					...
					<td class="lastpost ', $alternate_class, '">
						<div><a href="', $topic['last_post']['href'], '">', $txt['last_post'], '</a>  ', $txt['by'], ' ', $topic['last_post']['member']['link'], '</div>
						<div class="board_lastpost_time">', $txt['on'], ' ', date('F j, Y, g:i A', $topic['last_post']['timestamp']) ,'</div>
					</td>';
	...
	if (!$context['no_topic_listing'])
	{
		echo '
			<p class="floatleft smalltext">', !empty($modSettings['enableParticipation']) && $context['user']['is_logged'] ? '
				<img src="' . $settings['images_url'] . '/thread_icons/new.png" alt="" align="middle" /> New Content<br />' : '', '
				<img src="' . $settings['images_url'] . '/thread_icons/sticky.png" alt="" align="middle" /> Pinned Topic<br />
				<img src="' . $settings['images_url'] . '/thread_icons/locked.png" alt="" align="middle" /> Locked Topic<br />
			</p>
			<br class="clear" />';
	}
