...
function template_show_month_grid($grid_name)
{
	...
	if (empty($calendar_data['disable_day_titles']))
	{
		...
		foreach ($calendar_data['week_days'] as $day)
		{
			...
			echo '
						<th class="days" scope="col" ', $calendar_data['size'] == 'small' ? 'style="font-size: x-small;"' : '', '>', !empty($calendar_data['short_day_titles']) ? ($smcFunc['substr']($txt['days'][$day], 0, 1)) : $txt['days'][$day];
	...
	foreach ($calendar_data['weeks'] as $week)
	{
		...
		foreach ($week['days'] as $day)
		{
			...
			echo '
						<td style="height: ', $calendar_data['size'] == 'small' ? '20' : '100', 'px; padding: 2px;', $calendar_data['size'] == 'small' ? 'font-size: x-small;' : '', '" class="', $day['is_today'] ? 'calendar_today' : 'windowbg', ' days ', !empty($day['events']) ? 'calendar_hasEvent' : '' ,'">';
			...
			if (!empty($day['day']))
			{
				...
				{
					echo '
							', !empty($day['events'][0]['href']) ? '<a href="'. $day['events'][0]['href'] .'">' : '' ,'
								', $day['day'] ,'
							', !empty($day['events'][0]['href']) ? '</a>' : '';
				}
